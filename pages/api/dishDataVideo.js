...
export default async function handler(req, res) {
  ...
  try {
    ...
    const response = await fetch(
      `https://geri.biogrouppaca.local/geri_connect/public/api/v1/sessionrecords?session_uuid=${param2}`,
      {
        agent: httpsAgent,
        headers: {
    ...
    const response2 = await fetch(
      `https://geri.biogrouppaca.local/geri_connect/public/api/v1/dishrecords?dish_uuid=${videoID}`,
      {
        agent: httpsAgent,
        headers: {
          "Content-Type": "application/json",
