...
function Reader() {
  ...
  function onClose(status) {
    ...
    setShowModal(status);
    // Redirect to the next step
    // window.location.href = `https://geri.biogrouppaca.local/files/video.json/${url}/well01_zid99.mp4`;
    // window.location.href = `https://geri.biogrouppaca.local/files/video.json/${url}`
    // router.push(`${url}`);
  }
